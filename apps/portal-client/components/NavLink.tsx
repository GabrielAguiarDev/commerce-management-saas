...
/**
 * Um destino do portal, como link de verdade.
 *
 ...
 * esperando na frente do balcão. O `<Link>` anuncia o destino: o Next
 * pré-carrega as rotas dos links visíveis, e no clique a resposta já está na
 * mão.
 *
 * O que ele NÃO abandona são os efeitos de sair de uma tela, que sempre moraram
 * no `irPara`: fechar a gaveta do celular, o menu de linha e as notificações.
 * Como o `<Link>` navega por conta própria, quem os roda é o `onClick`. Ver
 * `PortalProvider`.
 *
  ...
  return (
    <Link
      ...
      onClick={(e) => {
        ...
        // Abrir em nova aba não é sair desta tela: nada a fechar. O navegador
        // cuida do resto.
        if (e.metaKey || e.ctrlKey || e.shiftKey || e.altKey) return;

        a.beforeNavigate(href);
      }}
    />
  );
}
