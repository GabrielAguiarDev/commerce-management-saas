 ...
 */
type Traco = ["rect" | "circle" | "path", Record<string, string | number>];

const TRACOS: Record<ModuloKey, Traco[]> = {
  dashboard: [
...
export function IconeModulo({ modulo, tamanho = 20 }: { modulo: ModuloKey; tamanho?: number }) {
  return (
    <svg
      width={tamanho}
      height={tamanho}
      ...
      aria-hidden
    >
      {TRACOS[modulo].map(([tipo, attrs], i) => {
        if (tipo === "rect") return <rect key={i} {...attrs} />;
        if (tipo === "circle") return <circle key={i} {...attrs} />;
